...
function login() {
    const matchedUser = users.find((user) => user.username == username.value && user.password == password.value);
    ...
    if (matchedUser) {
        ...
        toast.add({ severity: 'success', summary: 'Login Berhasil', detail: 'Berhasil login', life: 3000 });
        localStorage.setItem('authData', JSON.stringify(dataToStore));
        router.push('/dashboard');
    } else {
        toast.add({ severity: 'error', summary: 'Login Gagal', detail: 'Username atau Password salah', life: 3000 });
    }
}
</script>
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-2xl font-medium mb-4">Selamat Datang!</div>
                        <span class="text-muted-color font-medium">Sign in untuk melanjutkan</span>
                    </div>

                    <div>
                        <label for="username" class="block text-surface-900 dark:text-surface-0 text-l font-medium mb-2">Username</label>
                        <InputText id="username" type="text" placeholder="Username" class="w-full md:w-[30rem] mb-8" v-model="username" />

                        <label for="password" class="block text-surface-900 dark:text-surface-0 font-medium text-l mb-2">Password</label>
                        ...
                        <!-- <Button label="Sign In" class="w-full mt-6" as="router-link" @click="login"></Button> -->
                        <Button label="Sign In" class="w-full mt-6" @click="login"></Button>
                    </div>
                </div>
            </div>
        </div>
